...
highp float linearizeDepthLog(highp float depth,highp float near,highp float far){
...
highp float orthographicDepthToViewZ(highp float depth,highp float near,highp float far){
...
vec3 getViewNormal(sampler2D depth,vec2 uv,float near,float far,mat4 projMat){
    ...
    vec3 dpdx=(dl<dr)? ce-getViewPosition(l1,(uv-vec2(1.0/depthres.x,0.0)),near,far,projMat).xyz:-ce+getViewPosition(r1,(uv+vec2(1.0/depthres.x,0.0)),near,far,projMat).xyz;
    vec3 dpdy=(db<dt)? ce-getViewPosition(b1,(uv-vec2(0.0,1.0/depthres.y)),near,far,projMat).xyz:-ce+getViewPosition(t1,(uv+vec2(0.0,1.0/depthres.y)),near,far,projMat).xyz;
...
vec3 getWorldNormal(sampler2D depth,vec2 uv,float near,float far,mat4 projMat,mat4 worldMat){
    ...
    vec3 dpdx=(dl<dr)? ce-getWorldPosition(l1,(uv-vec2(1.0/depthres.x,0.0)),near,far,projMat,worldMat).xyz:-ce+getWorldPosition(r1,(uv+vec2(1.0/depthres.x,0.0)),near,far,projMat,worldMat).xyz;
    vec3 dpdy=(db<dt)? ce-getWorldPosition(b1,(uv-vec2(0.0,1.0/depthres.y)),near,far,projMat,worldMat).xyz:-ce+getWorldPosition(t1,(uv+vec2(0.0,1.0/depthres.y)),near,far,projMat,worldMat).xyz;
    return normalize(cross(dpdx,dpdy));
}
...
float outline(sampler2D tColor,sampler2D tInfo,vec2 uv,float outlineWidth,float scale,vec3 idRange,vec3 depthRange,vec3 normalRange,float rangeSmoothMargin,float idMinScale,vec2 distanceFadeRange){
    vec2 offset=1.0/vec2(textureSize(tInfo,0))*outlineWidth*scale;
    float contributions[5];
    float faceIds[5];
    float distances[5];
    vec3 normals[5];
    vec4 n=texture(tInfo,uv+offset*vec2(0,0));
    contributions[0]=n.a;
    distances[0]=1.0-n.r;
    normals[0]=decodeNormalSpheremap(n.gb);
    faceIds[0]=texture(tColor,uv+offset*vec2(0,0)).a;
    n=texture(tInfo,uv+offset*vec2(-1,0));
    contributions[1]=n.a;
    distances[1]=1.0-n.r;
    normals[1]=decodeNormalSpheremap(n.gb);
    faceIds[1]=texture(tColor,uv+offset*vec2(-1,0)).a;
    n=texture(tInfo,uv+offset*vec2(1,0));
    contributions[2]=n.a;
    distances[2]=1.0-n.r;
    normals[2]=decodeNormalSpheremap(n.gb);
    faceIds[2]=texture(tColor,uv+offset*vec2(1,0)).a;
    n=texture(tInfo,uv+offset*vec2(0,-1));
    contributions[3]=n.a;
    distances[3]=1.0-n.r;
    normals[3]=decodeNormalSpheremap(n.gb);
    faceIds[3]=texture(tColor,uv+offset*vec2(0,-1)).a;
    n=texture(tInfo,uv+offset*vec2(0,1));
    contributions[4]=n.a;
    distances[4]=1.0-n.r;
    normals[4]=decodeNormalSpheremap(n.gb);
    faceIds[4]=texture(tColor,uv+offset*vec2(0,1)).a;
    float centerId=faceIds[0];
    float centerDistance=distances[0];
    ...
    vec2 distanceVariation=vec2((distances[1]-centerDistance)-(distances[2]-centerDistance),(distances[3]-centerDistance)-(distances[4]-centerDistance));
    ...
    float distanceVariationTotal=sqrt((distanceVariation.x*distanceVariation.x)+(distanceVariation.y*distanceVariation.y));
    ...
    float depthContribution=fit(fit(distanceVariationTotal,depthRange.x,depthRange.y,0.0,1.0),depthLimit,depthLimit+rangeSmoothMargin,0.0,1.0);
    float minDistance=centerDistance;
...
#pragma unroll_loop_end
    ...
    centerContribution*=fit(-getViewZ(minDistance,uCameraNear,uCameraFar),distanceFadeRange.x,distanceFadeRange.y,1.0,0.0);
